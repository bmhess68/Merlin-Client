...
On a fresh Debian 12+ or Ubuntu 24.04+ machine, one line:

```bash
curl -fsSL https://raw.githubusercontent.com/bmhess68/Merlin-Client/main/scripts/install.sh | sudo bash
```

That clones the repo to `/opt/merlin-edge`, installs Docker and Tailscale,
prompts for the site slug and Tailscale auth (or login URL), builds and
starts the four-container stack, and prints the box's LAN and tailnet URLs.

For non-interactive provisioning (fleet rollouts, cloud-init, PXE):
...
config. Without `TAILSCALE_AUTH_KEY`, `tailscale up` prints a one-time login
URL the installer must click.

If you'd rather clone first (to inspect or work from a branch):

```bash
sudo git clone https://github.com/bmhess68/Merlin-Client.git /opt/merlin-edge
sudo bash /opt/merlin-edge/scripts/install.sh
```

After install finishes, open the installer UI at the URL it prints
(typically `http://<box-ip>:8080`). Enter the cloud playback hostname,
confirm the auto-detected tailnet hostname, then add cameras one at a time.
...
```
.
├── compose.yaml             # 4 services + log rotation
├── data/
...
├── playback-shim/
│   ├── Dockerfile           # Flask proxy in front of mediamtx :9996
│   └── app.py               # injects Basic auth from ?user=&pass= query string
├── relay/
